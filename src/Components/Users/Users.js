...
const Users = (props) => {
  ...
  useEffect(() => {
    if (relultUsers.length) setUsersCount(relultUsers.length)
    if (relultUsers.length && relultUsers.length < users.length)
      setPortionUsers(
        relultUsers.slice((currentPage - 1) * pageSize, currentPage * pageSize)
      )
  ...
  if (users && usersCount < 0) setUsersCount(users.length)
  ...
  const onSearch = (value) => {
    setRelultUsers(
      users.filter((val) =>
        val.displayName.toLocaleLowerCase().includes(value.toLocaleLowerCase())
      )
    )
  }
  return (
    <div className="users">
      ...
      {currentPage === 1 && portionUsers.length < pageSize ? null : (
        <Pagination
