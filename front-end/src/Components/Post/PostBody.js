...
const PostBody = (props) => {
  ...
  if (!autorData) autorData = getUserByID(props.users, props.uid)
  return (
    <>
      <div className="post__title">
        ...
        <div className="post__data w-100">
          {props.autor ? (
            <div className="post__name">{autorData.displayName}</div>
          ) : (
            <Link
              to={
                props.myId === props.uid
        ...
        {(props.myId === props.uid || !props.post) && (
          <div>
            <button
              ...
              type="button"
              onClick={() => ref.current.classList.toggle('d-block')}
            >
              More
            </button>
      ...
      <div className="post__text">{props.postText}</div>
      {props.src ? (
